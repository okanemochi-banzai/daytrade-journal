  ...
  for (const csv of csvs) {
    ...
    for (const code of tickers) {
      ...
      try {
        ...
        // Filter to target date + 1 prior trading day for BB warmup
        const targetDate = csv.date; // YYYY-MM-DD
        const targetDateObj = new Date(targetDate + 'T00:00:00Z');
        const oneDayMs = 86400 * 1000;
        // Keep candles from 1 day before target through target day end (JST)
        const filteredCandles = data.candles.filter(c => {
          const jst = new Date((c.t + 9 * 3600) * 1000);
          const dateStr = jst.toISOString().slice(0, 10);
          // Include target date and the most recent prior trading day for BB warmup
          const candleDate = new Date(dateStr + 'T00:00:00Z');
          const diffDays = (targetDateObj - candleDate) / oneDayMs;
          return diffDays >= 0 && diffDays <= 4; // up to 4 days back for BB warmup
        });
        const dayCandles = filteredCandles.filter(c => {
          const jst = new Date((c.t + 9 * 3600) * 1000);
          return jst.toISOString().slice(0, 10) === csv.date;
        });
        if (dayCandles.length === 0) {
          console.log(`  ⚠️  ${code}: no candles for ${csv.date} (got ${data.candles.length} total in fetch)`);
          totalFailed++;
          continue;
        }
        await fs.writeFile(outPath, JSON.stringify({
          ...
          candles: filteredCandles, // Save target day + warmup only
        }, null, 0));
        console.log(`  ✓ ${code}: ${dayCandles.length} day-candles, ${filteredCandles.length} total (incl. warmup)`);
        totalNew++;
